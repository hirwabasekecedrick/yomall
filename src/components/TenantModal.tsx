...
import { FileText, ShieldCheck, Banknote, Mail, Phone, Calendar, Download, Ellipsis } from 'lucide-react';
...
export default function TenantModal({ show, onClose, tenant, profile, onSave }) {
  ...
  const mockDocuments = [
    { icon: <FileText size={16} />, name: 'Lease Agreement', sub: `Signed ${localProfile.leaseStart}` },
    { icon: <ShieldCheck size={16} />, name: localProfile.insurance?.fileName || 'Insurance Certificate', sub: localProfile.insurance?.expiry ? `Expires ${localProfile.insurance.expiry}` : 'Not uploaded' },
    { icon: <Banknote size={16} />, name: 'Deposit Receipt', sub: `RWF ${localProfile.deposit?.toLocaleString()}` },
  ];
      ...
      <div className="tm-panel" onClick={e => e.stopPropagation()}>
        <div className="tm-header">
          ...
          <div className="tm-contact-row">
            <div className="tcr-item" style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Mail size={12} /> {localProfile.email}</div>
            <div className="tcr-item" style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Phone size={12} /> {tenant.phone}</div>
            <div className="tcr-item" style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Calendar size={12} /> Lease start: {localProfile.leaseStart}</div>
          </div>
        </div>
            ...
            <div className="tm-panel active">
              {mockReceipts.map((r, i) => (
                <div key={i} className="tm-doc-row" style={{ justifyContent: 'space-between', alignItems: 'center', padding: '14px 12px', borderBottom: '1px solid var(--color-ink-100)' }}>
                  ...
                  <button className="btn-sm btn-outline" style={{ display: 'flex', alignItems: 'center', gap: 4 }} onClick={() => alert(`Downloading ${r.name}`)}>
