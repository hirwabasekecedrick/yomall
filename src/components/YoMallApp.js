...
/* ─────────────── MAIN APP ─────────────── */
export default function YoMallApp({ renderLogin, initialRole }) {
  return (
    <ToastProvider>
      <AppContent renderLogin={renderLogin} initialRole={initialRole} />
    </ToastProvider>
  );
}

const defaultViewForRole = (role) => (role === 'superadmin' ? 'console' : role === 'tenant' ? 't-overview' : 'overview');

function AppContent({ renderLogin, initialRole }) {
  const toast = useToast();
  const router = useRouter();
  const [loggedIn, setLoggedIn] = useState(Boolean(initialRole));
  const [currentRole, setCurrentRole] = useState(initialRole || 'landlord');
  const [currentView, setCurrentView] = useState(defaultViewForRole(initialRole));
  ...
  const handleLogin = (role) => {
    ...
    setCurrentRole(role);
    setCurrentView(defaultViewForRole(role));
    setViewHistory([]);
    setLoggedIn(true);
  };

  const handleLogout = () => {
    if (initialRole) { router.push('/'); return; }
    setLoggedIn(false);
  ...
  if (!loggedIn) return renderLogin ? renderLogin(handleLogin) : <LoginScreen onLogin={handleLogin} />;
  ...
  const SIDEBARS = { landlord: LandlordSidebar, tenant: TenantSidebar, superadmin: SuperadminSidebar };
  const TOPBARS = { landlord: LandlordTopbar, tenant: TenantTopbar, superadmin: SuperadminTopbar };
  const RoleSidebar = SIDEBARS[currentRole];
  const RoleTopbar = TOPBARS[currentRole];

  return (
    <div style={{display:'flex',minHeight:'100vh'}}>
      <RoleSidebar currentView={currentView} setView={navigate} onLogout={handleLogout} mobileOpen={navOpen} setMobileOpen={setNavOpen} />
