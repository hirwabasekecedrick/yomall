...
export default function LandlordTopbar({ title, subtitle, canGoBack, onBack, onNavigate, onMenu }) {
      ...
      <div className="h-[66px] shrink-0 bg-[#FFFEFB] border-b border-line flex items-center justify-between gap-[10px] px-[14px] sm:px-[26px] sticky top-0 z-10">
        <div className="flex items-center gap-[10px] sm:gap-[12px] min-w-0">
          {onMenu && (
            <button
              ...
              className="lg:hidden w-[34px] h-[34px] shrink-0 rounded-[9px] border border-line bg-white flex items-center justify-center cursor-pointer text-[15px] text-ink-600"
            >
              ☰
            </button>
          )}
        ...
        <div className="flex items-center gap-[8px] sm:gap-[12px] shrink-0">
          <button
            ...
            className={`md:hidden w-[34px] h-[34px] rounded-[9px] border flex items-center justify-center cursor-pointer text-[13px] ${showSearch ? 'border-forest-500 bg-forest-100 text-forest-700' : 'border-line bg-white'}`}
          >
            🔍
          </button>
      ...
      {showSearch && (
        <div className="md:hidden bg-[#FFFEFB] border-b border-line px-[14px] py-[10px] sticky top-[66px] z-10">
          <div className="flex items-center gap-[7px] bg-cream-100 border border-line rounded-[9px] p-[8px_11px] text-[12.5px] text-ink-600">
            🔍 <input autoFocus placeholder="Search tenant, unit, order..." className="border-none bg-transparent outline-none text-[12.5px] w-full text-ink-900 font-sans" />
          </div>
        </div>
      )}
    </>
