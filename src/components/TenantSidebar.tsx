...
import { LayoutDashboard, Store, Tag, Package, Wallet, ScrollText, MessageSquare, Wrench, BookOpen, LogOut } from 'lucide-react';

export default function TenantSidebar({ currentView, setView, onLogout, mobileOpen, setMobileOpen }: { currentView: string; setView: (v: string) => void; onLogout: () => void; mobileOpen?: boolean; setMobileOpen?: (v: boolean) => void }) {
    ...
    <aside className="w-full sm:w-[236px] shrink-0 h-full bg-gradient-to-b from-forest-900 to-forest-800 text-[#EFF6EF] flex flex-col p-[22px_14px] overflow-y-auto">
      <div className="flex items-center gap-[10px] p-[4px_8px_22px_8px]">
        ...
        <div>
          ...
          <div className="text-[10.5px] text-[#AFC9B4] tracking-[.4px] uppercase mt-[1px]">Tenant portal</div>
        </div>
      </div>
      ...
      <div className="flex-1">
        ...
        <nav className="flex flex-col gap-[2px]">
          ...
          <NavItem view="storefront" icon={<Store size={17} />} label="My Storefront" />
          <NavItem view="yodeals" icon={<Tag size={17} />} label="Post to yoDeals" />
          <NavItem view="orders" icon={<Package size={17} />} label="My Orders" />
          <NavItem view="rent" icon={<Wallet size={17} />} label="My Rent" />
          <NavItem view="lease" icon={<ScrollText size={17} />} label="My Lease" />
          <NavItem view="messages" icon={<MessageSquare size={17} />} label="Messages" />
        </nav>
        <NavGroupLabel>Support</NavGroupLabel>
        <nav className="flex flex-col gap-[2px]">
          <NavItem view="staff" icon={<Wrench size={17} />} label="Maintenance Directory" />
      ...
      <div className="flex items-center justify-between p-[4px_11px_0]">
        <span className="inline-flex items-center gap-[4px] px-[8px] py-[3px] rounded-full bg-white/10 text-white text-[9px] font-bold uppercase tracking-[.5px]">Tenant</span>
